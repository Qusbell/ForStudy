...
public:
	...
	bool Packing_OnConnect(PacketOnConnect& packet,
	...
	void Packing_AssignID(PacketAssignID& packet,
		const int ID)
	{
	...
	bool Packeting_RequestChat(PacketRequestChat& packet,
	...
	bool Packing_BroadcastChat(PacketBroadcastChat& packet,
		const std::string& name,
		const std::string& chat)
	{
		if (chat.empty()) { return false; }
