...
void TreeBillboardsApp::UpdateObjectCBs(const GameTimer& gt)
{
	...
	auto& Ritems = mRenderItemManager.GetAllRitems();
	for(auto& e : Ritems)
	{
...
void TreeBillboardsApp::BuildPSOs()
{
	...
	auto& shaders = mShaderManager.GetShaders(); // 맵 참조 획득
	...
	opaquePsoDesc.InputLayout = { mShaderManager.GetStdInputLayout().data(), (UINT)mShaderManager.GetStdInputLayout().size() };
	//opaquePsoDesc.pRootSignature = mRootSignature.Get();
	opaquePsoDesc.pRootSignature = mShaderManager.GetRootSignature();
	opaquePsoDesc.VS = 
	{ 
	...
	treeSpritePsoDesc.InputLayout = { mShaderManager.GetTreeSpriteInputLayout().data(), (UINT)mShaderManager.GetTreeSpriteInputLayout().size() };
...
void TreeBillboardsApp::BuildRenderItems()
{
	...
	auto& allRitems = mRenderItemManager.GetAllRitems();
    ...
    mRenderItemManager.SetWavesRitem(wavesRitem.get());
	mRenderItemManager.GetRitemLayer(RenderLayer::Transparent).push_back(wavesRitem.get());
	...
	mRenderItemManager.GetRitemLayer(RenderLayer::Opaque).push_back(gridRitem.get());
	...
	mRenderItemManager.GetRitemLayer(RenderLayer::AlphaTested).push_back(boxRitem.get());
	...
	mRenderItemManager.GetRitemLayer(RenderLayer::AlphaTestedTreeSprites).push_back(treeSpritesRitem.get());
